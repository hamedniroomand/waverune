...
test("snr decreases as noise grows", () => {
  ...
  const noisy = (amp: number) => { const b = Float32Array.from(a); for (let i = 0; i < n; i++) b[i] += amp * (Math.random() * 2 - 1); return b; };
  expect(calculateAudioMetrics(a, noisy(0.001)).snr).toBeGreaterThan(calculateAudioMetrics(a, noisy(0.1)).snr);
});

...
// The seed derivation used `new Bun.CryptoHasher('sha256', key)`, which is
// HMAC-SHA256 with `key`, not a plain hash over the concatenation. Keys and
// chip sequences of existing watermarks depend on this exact function.
...
// A fixed vector, so a change in derivation shows up without Bun present.

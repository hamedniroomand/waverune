...
test("rng is deterministic for a seed and varies across seeds", () => {
  const a = makeRng(deriveSeed("secret", "cells")), b = makeRng(deriveSeed("secret", "cells"));
...
test("cells are balanced across bits", () => {
  ...
  const counts = new Array(56).fill(0);
...
test("assignment is reproducible", () => {
  const a = assignCells("k", 20, 8, 16), b = assignCells("k", 20, 8, 16);
